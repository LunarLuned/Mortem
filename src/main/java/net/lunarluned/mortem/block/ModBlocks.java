...
public class ModBlocks {
    ...
    public static final Block APPLE_PIE = registerBlockWithItemThatStacksToOne("apple_pie",
            new ApplePieBlock(BlockBehaviour.Properties.of().forceSolidOn().strength(0.5F).sound(SoundType.WOOL).pushReaction(PushReaction.DESTROY).setId(ResourceKey.create(Registries.BLOCK, Identifier.tryBuild(Mortem.MOD_ID, "apple_pie")))));
    ...
    public static final Block HARDENED_DEEPSLATE_IRON_ORE = registerBlock("hardened_deepslate_iron_ore",
            new HardenedIronOre(BlockBehaviour.Properties.of().requiresCorrectToolForDrops().setId(ResourceKey.create(Registries.BLOCK, Identifier.tryBuild(Mortem.MOD_ID, "hardened_deepslate_iron_ore")))
                    .strength(12f).explosionResistance(4).sound(SoundType.DEEPSLATE)));

    private static Block registerBlock(String name, Block block) {
        registerBlockItem(name, block);
        return Registry.register(BuiltInRegistries.BLOCK, Identifier.tryBuild(Mortem.MOD_ID, name), block);
    }
    ...
    private static void registerBlockItem(String name, Block block) {
        Registry.register(BuiltInRegistries.ITEM, Identifier.tryBuild(Mortem.MOD_ID, name),
                new BlockItem(block, new Item.Properties().setId(ResourceKey.create(Registries.ITEM, Identifier.tryBuild(Mortem.MOD_ID, name))).useBlockDescriptionPrefix()));
    }

    private static void registerBlockItemStacksTo1(String name, Block block) {
        Registry.register(BuiltInRegistries.ITEM, Identifier.tryBuild(Mortem.MOD_ID, name),
                new BlockItem(block, new Item.Properties().stacksTo(1).setId(ResourceKey.create(Registries.ITEM, Identifier.tryBuild(Mortem.MOD_ID, name))).useBlockDescriptionPrefix()));
    }
